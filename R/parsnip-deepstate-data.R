...
make_deep_state <- function() {

    # SETUP
    model <- "deep_state"
    mode  <- "regression"
    eng   <- "gluonts_deepstate"

    parsnip::set_new_model(model)
    parsnip::set_model_mode(model, mode)

    # deep_state: regression ----
    ...
    parsnip::set_model_arg(
        ...
        parsnip      = "add_trend",
        original     = "add_trend",
        func         = list(pkg = "modeltime.gluonts", fun = "add_trend"),
        has_submodel = FALSE
    )
