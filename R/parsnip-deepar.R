...
#' - __Pytorch:__ "torch". Requires `pytorch` and `pytorch-lightning`. Install with `install_gluonts(include_pytorch = TRUE)`.
#'
...
#'
#' @inheritParams deepar_fit_impl
#' @param hidden_size Hidden units
#' @param num_feat_dynamic_real Number of dynamic numeric features
...
deepar_torch_fit_impl <- function(x, y, freq, prediction_length, id,
    ...
    if (is.null(cardinality)) cardinality <- reticulate::py_none()
    if (is.null(embedding_dimension)) embedding_dimension <- reticulate::py_none()
    if (distr_output == "default") distr_output <- pkg.env$gluonts$torch$modules$distribution_output$StudentTOutput()
    if (is.null(lags_seq)) lags_seq <- reticulate::py_none()
    if (is.null(time_features)) time_features <- reticulate::py_none()
    # if (is.null(imputation_method)) imputation_method <- reticulate::py_none()
    ...
    args_test <- names(dot_list) %in% args_not_used
    found_args_not_used <- any(args_test)
    if (found_args_not_used) {
        message("Found parameters that are not used with Torch Trainer:")
        message(" ", stringr::str_c(names(dot_list)[args_test], collapse = TRUE), "\n")
        message("Removing from training operation.")
        message("----\n")

        # Update dot_list to only contain args not in the unused list
        dot_list <- dot_list[names(dot_list)[!args_test]]
    ...
    # Construct Torch Trainer
    trainer <- reticulate::dict(
        c(
            # Defined Args
            list(
                max_epochs = as.integer(epochs)
            ),
            # Undefined Args
            dot_list
        )
    ...
    model_spec <- pkg.env$gluonts$torch$model$deepar$DeepAREstimator(
        freq                   = freq,
        prediction_length      = prediction_length,
        ...
        num_feat_dynamic_real  = as.integer(num_feat_dynamic_real),
        num_feat_static_cat    = as.integer(num_feat_static_cat),
        num_feat_static_real   = as.integer(num_feat_static_real),
